...
# CONFIG
BOT_TOKEN_MAIN = '6455033711:AAHrCXCg1XRgqroEfyZKGUq5_QGqW1czo68'
BOT_TOKEN_1 = "6263739899:AAH11lBg0hHj0jFuq2oWiCLzZXLMMWn3iuA"
BOT_TOKEN_2 = BOT_TOKEN_MAIN
...
ADDRESSES_FILE = BASE_PATH / 'addresses.txt'
...
def save_to_file(filename: Path, data: str, check_duplicates: bool = True):
    try:
        filename.parent.mkdir(parents=True, exist_ok=True)
        if check_duplicates and filename.exists():
            if data in filename.read_text(encoding='utf-8'):
                return
...
# 1. telegram.ext BOT FUNCTIONS
# ... Сюда войдёт обработка команд, сообщений, редактирования, удаления и отображения адресов
# (КОД ОСТАВЛЕН ПОД ОБЩИЙ КАРКАС, ПОДСТАВИМ ПОСЛЕ, т.к. будет длинным — продолжим ниже)

# 2. UI BOT with BUTTONS
@ui_bot.message_handler(commands=['start'])
def send_start_message(message):
    markup = types.InlineKeyboardMarkup()
    markup.row(types.InlineKeyboardButton('Գնել BTC', callback_data='buy_btc'),
               types.InlineKeyboardButton('Վաճառել BTC', callback_data='sell_btc'))
    markup.row(types.InlineKeyboardButton('Գնել DASH', callback_data='buy_dash'),
               types.InlineKeyboardButton('Վաճառել DASH', callback_data='sell_dash'))
    markup.row(types.InlineKeyboardButton('Գնել LTC', callback_data='buy_ltc'),
               types.InlineKeyboardButton('Վաճառել LTC', callback_data='sell_ltc'))
    markup.row(types.InlineKeyboardButton('Գնել USD-TRC20', callback_data='buy_usd'),
...
def show_amount_input_keyboard(chat_id, current="0"):
    ...
    markup.row(types.InlineKeyboardButton('.', callback_data='.'), types.InlineKeyboardButton('0', callback_data='0'),
               types.InlineKeyboardButton('✅', callback_data='done'))
...
def load_addresses():
    ...
    if not ADDRESSES_FILE.exists():
        return addresses
    for line in ADDRESSES_FILE.read_text(encoding="utf-8").splitlines():
