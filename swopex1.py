...
# Пути — используем папку, где лежит этот файл
...
async def generate_and_send_image():
    try:
        ...
        if not os.path.exists(font_path):
            print(f"[BtcKiosk] font not found: {font_path} (будет использован системный шрифт)")

        # Рендер чека (чёрный фон + белый шрифт)
        img = Image.open(image_path)
        dash = float(lines[3].replace(",", "."))
        s = f"{dash:.7f}"
